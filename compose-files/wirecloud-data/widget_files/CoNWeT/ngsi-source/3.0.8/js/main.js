...
(function () {
    ...
    var doInitialSubscription = function doInitialSubscription() {        
        ...
        if (MashupPlatform.prefs.get('use_owner_credentials')) {            
            request_headers['X-FIWARE-OAuth-Token'] = 'Bearer ' + MashupPlatform.context.get('fiware_token');            
        ...
        // Add headers when creating the NGSI connection        
        ...
        var options = {            
            ...
            onSuccess: function (data) {                
                ...
                this.refresh_interval = setInterval(refreshNGSISubscription.bind(this), 1000 * 60 * 60 * 2);  // Refresh every 2 hours                
                doInitialQueries.call(this, entityIdList);                
                window.addEventListener("beforeunload", function () {                    
                    this.connection.cancelSubscription(this.subscriptionId, {                        
                        onSuccess: function () {                            
                            MashupPlatform.operator.log("Subscription cancelled successfully", MashupPlatform.log.INFO);                        
                        },                        
                        onFailure: function () {                            
    ...
    var refreshNGSISubscription = function refreshNGSISubscription() {        
        if (this.subscriptionId) {            
            ...
            var options = {                
                onSuccess: function () {                    
                    MashupPlatform.operator.log("Subscription refreshed successfully", MashupPlatform.log.INFO);                
                },                
                onFailure: function () {                    
    ...
    var handlerPreferences = function handlerPreferences(new_values) {        
        ...
        if (this.subscriptionId != null) {            
            this.connection.cancelSubscription(this.subscriptionId, {                
                onSuccess: function () {                    
                    MashupPlatform.operator.log("Old subscription has been cancelled successfully", MashupPlatform.log.INFO);                
                },                
                onFailure: function () {                    
